...
flags.DEFINE_integer('Auto_batch', 52, 'Batch size for training.')
...
flags.DEFINE_integer('numofclients', 4, 'Number of epochs to train for.')
...
# baray Module aval
  ...
  for m in range(FLAGS.numofclients):
    with strategy.scope():
      model_1.append(model_lib.Module_1(num_classes))
  ...
  if FLAGS.mode == 'eval':
    ...
    with strategy.scope():
      ...
      ds1.append(data_lib.build_distributed_dataset(builder, 64, True, strategy, topology))
      ds2.append(data_lib.build_distributed_dataset(builder, 52, False, strategy, topology))
      ...
      ds1.append(data_lib.build_distributed_dataset(builder, 64, True, strategy, topology)) 
      ds2.append(data_lib.build_distributed_dataset(builder, 52, True, strategy, topology))  
      ...
      for m in range(FLAGS.numofclients):
        optimizer_1.append(model_lib.build_optimizer(0.001))
    ...
    def single_step_1(features, labels, mod1, opt1):
      with tf.GradientTape() as tape:
        should_record = tf.equal((opt1.iterations + 1) % steps_per_loop_1, 0)
        # with tf.summary.record_if(should_record):
        #   tf.summary.image('image', features[:, :, :, :3], step=optimizer_1.iterations + 1)
        
        hdd, fea = mod1(features, training=True)
        # flops(mod1)
        loss = None
        if hdd is not None:
          outputs = hdd          
        ...
        for var in mod1.trainable_variables:
          logging.info(var.name)
        grads = tape.gradient(loss, mod1.trainable_variables)
        opt1.apply_gradients(zip(grads, mod1.trainable_variables))
    

    def single_step(features0, labels0, features1, labels1, features2, labels2, features3, labels3,
                features4, labels4, features5, labels5, features6, labels6, features7, labels7, features8, labels8,
                features9, labels9):
      ...
      with tf.GradientTape() as tape:
        ...
        with tf.summary.record_if(should_record):
          ...
          tf.summary.image('image', features0[:, :, :, :3], step=optimizer.iterations + 1)

        rep0 = model_1[0](features0, training=False)
        rep1 = model_1[1](features1, training=False)
        rep2 = model_1[2](features2, training=False)
        rep3 = model_1[3](features3, training=False)
        rep4 = model_1[4](features0, training=False)
        rep5 = model_1[5](features1, training=False)
        rep6 = model_1[6](features2, training=False)
        rep7 = model_1[7](features3, training=False)
        rep8 = model_1[8](features2, training=False)
        rep9 = model_1[9](features3, training=False)        
      ...
      with strategy.scope():
        ...
        def train_multiple_steps(iterator):
          for _ in tf.range(steps_per_loop_1):
            with tf.name_scope(''):
              images, labels = next(iterator)
              features, labels = images, {'labels': labels}
              strategy.run(single_step_1, (features, labels, model_1[m], optimizer_1[m]))

        global_step = optimizer_1[m].iterations
        ...
        while cur_step_1 < train_steps_1:
          ...
          for metric in all_metrics:
            metric.reset_states()
        # logging.info('Training 1 complete... for client', m)
        print('Training 1 complete... for client', m)
  ...
  # baray Module sevom
    ...
    with strategy.scope():
      @tf.function
      def train_multiple_steps(iterator0, iterator1, iterator2, iterator3, iterator4, iterator5, iterator6,
      iterator7, iterator8, iterator9):
        ...
        for _ in tf.range(steps_per_loop_3):
          ...
          with tf.name_scope(''):
            ...
            images, labels = next(iterator7); features8, labels8 = images, {'labels': labels}
            images, labels = next(iterator9); features9, labels9 = images, {'labels': labels}

            strategy.run(single_step, (features0, labels0, features1, labels1, features2, labels2, features3, labels3,
                features4, labels4, features5, labels5, features6, labels6, features7, labels7, features8, labels8,
                features9, labels9))
      
      global_step = optimizer.iterations
      cur_step_3 = global_step.numpy()
      iterator0 = iter(ds2[0]); iterator1 = iter(ds2[1]);iterator2 = iter(ds2[2]); iterator3 = iter(ds2[3])
      iterator4 = iter(ds2[4]); iterator5 = iter(ds2[5]);iterator6 = iter(ds2[6]); iterator7 = iter(ds2[7])
      iterator8 = iter(ds2[8]); iterator9 = iter(ds2[9])
